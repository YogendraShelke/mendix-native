...
@objc public class NativeOtaModule: NSObject {
  ...
  func download(_ config: [String: Any],
    ...
    if !fileManager.fileExists(atPath: otaDir) {
      do {
        ...
        reject(OTA_DOWNLOAD_FAILED, "Failed creating ota directories", error)
        return
      }
    }
    ...
    guard let runtimeUrl = MxConfiguration.runtimeUrl?.absoluteString else {
      reject(INVALID_RUNTIME_URL, "Runtime URL is not set.", nil)
      return
    }
    ...
    let downloadHandler = NativeDownloadHandler(
      ...
      failCallback: { error in
        reject(OTA_DOWNLOAD_FAILED, "OTA download failed.", error)
      }
    )
   ...
   * }
   */
  @objc(deploy:resolver:rejecter:)
  func deploy(_ config: [String: Any],
              resolver resolve: @escaping RCTPromiseResolveBlock,
              rejecter reject: @escaping RCTPromiseRejectBlock) {
    
    guard let otaDeploymentID = config[MANIFEST_OTA_DEPLOYMENT_ID_KEY] as? String else {
      reject(INVALID_DOWNLOAD_CONFIG, "Key otaDeploymentID is invalid.", nil)
      return
    }
    ...
    if !unzipped {
      ...
      reject(OTA_DEPLOYMENT_FAILED, "OTA deployment failed.", nil)
      return
    }
